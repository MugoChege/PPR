...
let nums_number: Int = number_from_list(list: nums);
nums.sort();
nums.reverse();
let highest_value_nums_number: Int = number_from_list(list: nums);

print(nums_number);
print(highest_value_nums_number);

var next_permutation: Array<Int> = [];
var running_number: Int = nums_number;

while running_number <= highest_value_nums_number{
	var temp_nums_list = number_to_list(number: running_number, original_list_len: len_nums);
	if (list_comparison(list1: temp_nums_list, list2: nums) && (running_number != nums_number)){
