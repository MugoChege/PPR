...
fn main (){
	...
	let nums_number: isize = number_from_list(nums.clone());
	nums.sort();
	nums.reverse();
	let highest_value_nums_number: isize = number_from_list(nums.clone());
	
	println!("The value of the number from the elements is:\t{}", nums_number);
	println!("The highest number from the elements is:\t{}", highest_value_nums_number);
	
	let mut running_number: isize = nums_number;
	let mut next_permutation: Vec<isize> = vec![0; len_nums as usize];
	
	while (running_number <= highest_value_nums_number){
		let temp_list: Vec<isize> = number_to_list(running_number, len_nums);
		
		if list_comparison (temp_list.clone(), nums.clone()) && (running_number != nums_number){
