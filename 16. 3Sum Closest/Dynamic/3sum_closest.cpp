...
vector<Result> sum_of_three_closest (vector<int> list, int target){
	vector<Result> out;
						...
						if ((a != b) && (a != c) && (b != c)){
							...
							if (true){
								...
								if (update_array){
									...
									out.push_back(Result(y, number_of_elements));
								}
							}
						}
					}
...
int main () {
	...
	vector<Result> res = sum_of_three_closest(nums, target);
