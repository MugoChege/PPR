...
nums.sort();
let lowest_value_nums_number: Int = number_from_list(list: nums);
nums.reverse();
...
while (running_number <= highest_value_nums_number){
	let temp_nums_list: Array<Int> = number_to_list(number: running_number, original_list_len: len_nums);
