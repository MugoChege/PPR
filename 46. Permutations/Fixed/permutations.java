import java.util.Arrays;

class permutations{
	public static int exponented (int base, int exponent){
		int out = 1;
		
		for (int i = 0; i < exponent; i++){
			out *= base;
		}
		
		return (out);
	}
	
	public static int factorial (int number){
		int out = 1;
		
		for (int i = number; i > 0; i--){
			out *= i;
		}
		
		return (out);
	}
	
	public static int number_from_list (int [] list){
		int len_list = list.length;
		int out = 0;
		int base = 10;
		
		for (int i = 0; i < len_list; i++){
			out += (list[i] * exponented(base, (len_list - 1 - i)));
		}
		
		return (out);
	}
	
	public static int [] number_to_list (int number, int len_list){
		int [] out = new int [len_list];
		
		for (int i = 0; i < len_list; i++){
			out[i] = 0;
		}
		
		int base = 10;
		int running_number = number;
		int out_cursor = len_list - 1;
		
		while ((running_number > 0) && (out_cursor >= 0)){
			int modulo = running_number % base;
			running_number -= modulo;
			running_number /= base;
			out[out_cursor] = modulo;
			out_cursor--;
		}
		
		return (out);
	}
	
	public static boolean list_comparison (int [] list1, int [] list2){
		int len_list1 = list1.length;
		int len_list2 = list2.length;
		
		if (len_list1 != len_list2){
			return (false);
		}
		
		boolean [] list1_bool_map = new boolean [len_list1];
		boolean [] list2_bool_map = new boolean [len_list2];
		
		for (int i = 0; i < len_list1; i++){
			list1_bool_map[i] = false;
			list2_bool_map[i] = false;
		}
		
		for (int i = 0; i < len_list1; i++){
			for (int j = 0; j < len_list1; j++){
				if ((list1_bool_map[i] == false) && (list2_bool_map[j] == false)){
					if (list1[i] == list2[j]){
						list1_bool_map[i] = true;
						list2_bool_map[j] = true;
						break;
					}
				}
			}
		}
		
		int bool_counter = 0;
		
		for (int i = 0; i < len_list1; i++){
			if (list1_bool_map[i] && list2_bool_map[i]){
				bool_counter += 1;
			}
		}
		
		return (len_list1 == bool_counter);
	}
	
	public static int appears (int element, int [] list){
		int len_list = list.length;
		int out = -1;
		
		for (int i = 0; i < len_list; i++){
			if (element == list[i]){
				out += 1;
			}
		}
		
		return (out);
	}
	
	//sort
	public static int [] reverse_list (int [] list){
		int len_list = list.length;
		int half_len_list = len_list / 2;
		
		for (int i = 0; i < half_len_list; i++){
			int x = list[i];
			
			list[i] = list[len_list - 1 - i];
			list[len_list - 1 - i] = x;
		}
		
		return (list);
	}
	
	public static void main (String [] args){
		int [] nums = {1,2,3,4};
		int len_nums = nums.length;
		
		Arrays.sort(nums);
		
		int lowest_value_nums_number = number_from_list(nums);
		nums = reverse_list(nums);
		int highest_value_nums_number = number_from_list(nums);
		
		System.out.print("The lowest number from the elements is:\t");
		System.out.println(lowest_value_nums_number);
		System.out.print("The highest number from the elements is:\t");
		System.out.println(highest_value_nums_number);
		
		int sample_size = factorial (len_nums);
		int unique = 0;
		
		for (int i = 0; i < len_nums; i++){
			boolean occ_code = false;
			
			if (i == 0){
				occ_code = true;
			} else {
				if (nums[i - 1] != nums[i]){
					occ_code = true;
				}
			}
			
			if (occ_code){
				int occurences = appears(nums[i], nums);
				if (occurences == 0){
					unique++;
				}
				
				if (occurences > 0){
					unique++;
					sample_size /= factorial(occurences + 1);
				}
			}
		}
		
		System.out.print("The number of unique elements are:\t");
		System.out.println(unique);
		System.out.print("The sample size of permutations expected is:\t");
		System.out.println(sample_size);
		
		int permutations[][] = new int [sample_size][len_nums];
		int running_number = lowest_value_nums_number;
		int permutations_cursor = 0;
		
		while (running_number <= highest_value_nums_number){
			int [] temp_array = new int [len_nums];
			
			for (int i = 0; i < len_nums; i++){
				temp_array[i] = 0;
			}
			
			temp_array = number_to_list(running_number, len_nums);
			
			if (list_comparison(temp_array, nums)){
				for (int i = 0; i < len_nums; i++){
					permutations[permutations_cursor][i] = temp_array[i];
				}
				permutations_cursor++;
			}
			
			running_number++;
		}
		
		for (int i = 0; i < permutations_cursor; i++){
			for (int j = 0; j < len_nums; j++){
				System.out.print(permutations[i][j]);
				
				if (j != (len_nums - 1)){
					System.out.print("\t");
				}
			}
			System.out.println();
		}
	}
}
